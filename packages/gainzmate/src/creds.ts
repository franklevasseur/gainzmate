export namespace telegram {
  export const prodBotToken = '6896170040:AAFdYMUpAoqMhZr2RKkbpT2ZXsxWLp-72IU'
  export const devBotToken = '6402478878:AAE-zzePKjgIl23G4VoP_S1StPaf4JoBzHU'
}

export namespace gsheets {
  export const spreadsheetId = '1d5Vgaixn8-QrBJmqUZifXWFB53bUWKlKAOerw6Y08A8'
  export const clientEmail = 'fleur-tmp-3@fleur-tmp.iam.gserviceaccount.com'
  export const privateKey =
    '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
}

export namespace digitalocean {
  export const region = 'nyc3'
  export const spaceName = 'gainzfiles'
  export const accessKey = 'DO00FFUNDYNL99GWZCM3'
  export const secretKey = 'e/0aOCYS83GKkd6cRpqe13gqSbEcnOh7RlwRAbYVh0U'
}
